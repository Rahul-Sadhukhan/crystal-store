...
@Slf4j
public class PrePMSummaryWorkbookReporter implements WorkbookReporter {
    ...
    @Getter
    private final String reportName = " - DOC Pre-PM Report";

    public PrePMSummaryWorkbookReporter(MetadataProperties metadataProperties, StoreReviewReport storeReviewReport, WorkbookUtil workbookUtil) {
    ...
    public String getTemplatePath() {
        return "report/templates/store-review-report/Crystal-PRE-PM-Template.xlsx";
    }
    ...
    public byte[] getWorkbook() {
        try (XSSFWorkbook workbook = new XSSFWorkbook(new ClassPathResource(getTemplatePath()).getInputStream())) {
            workbookUtil.addDataToCellsAndRows(workbook, storeReviewReport, metadataProperties, reviewManifests, assetDetailManifests, rackCellPositions, caseCellPositions);
    ...
    static {
        ...
        addReviewManifest(POST_PM_STORE_HEALTH_SCORE, STRING, report -> report.getStoreReviewDetails().getReviewerName(), reviewManifests);
        addReviewManifest(REVIEWER, STRING, report -> report.getStoreReviewDetails().getReviewerName(), reviewManifests);
    ...
    static {
        ...
        rackCellPositions.put(RECOMMENDATION_NOTES, 17);
    }
